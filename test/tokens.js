...
test( 'Tokens', t => {
    t.test( 'label.js', t => {
        ...
        t.test( 'label', t => {
            ...
            t.equal( label( {method:'info',params:[7]} ),   '[INFO] ', 'Info should have correct label and padding' );
            t.equal( label( {method:'debug',params:[7]} ),  '[DEBUG]', 'Debug should have correct label and padding' );
            t.equal( label( {method:'error',params:[-5]} ), '[ERROR]', 'Error should have correct label and padding' );
            t.equal( label( {method:'error',params:[]} ),   '[ERROR]', 'Error should have correct label and padding' );
            t.end();
        } );
    ...
    t.test( 'date.js', t => {
        t.equal(date({params:['dd.mm.yyyy HH:MM']}),dformat('dd.mm.yyyy HH:MM'));
        t.end();
    });
    t.end();
} );